...
use App\Traits\SearchTitleByFormIdTrait;
use Illuminate\Http\JsonResponse;

class DeploymentController extends Controller
{
    use ModelDetails, CommonControllerShowTrait, SearchTitleByFormIdTrait;
    protected $modelId;
    ...
    public function index(Request $request, FormListService $globalFormService)
    {
        try {
            ...
            $rules = [
                ...
                'reference_form_id' => ['nullable', 'integer', 'exists:forms,id', 'min:1'],
                'reference_details' => ['nullable', 'string', 'min:1'],
    ...
    public function filters(Request $request, DeploymentService $service)
    {
    ...
    public function show($id)
    {
        try {
            $record = $this->getModel()::with(
                ...
                'referenceForm:id,name,slug',
                'referenceDetail:id,request_title,sequence_no',
                'deploymentDetail',
                'qualityAssurances',
                'attachables'
            )->findOrFail($id);
    ...
    public function store(StoreDeploymentRequest $request, GlobalFormStoreService $globalFormStoreService, DeploymentService $service)
    {
        try {
            $response = '';
            ...
            $validatedData['form_id'] = (int) $validatedData['reference_form_id'];
    ...
    public function update(UpdateDeploymentRequest $request, GlobalFormStoreService $globalFormStoreService, DeploymentService $service, $id)
    {
        try {
            $response = '';
            $validatedData = $request->validated();
    ...
    public function deploymentStatusChange(Request $request, Deployment $deployment): JsonResponse
    {
        if ($deployment->status != 'Approved') {
            return Helper::sendError('Record not found or the status is not Approved', [], 400);
        }

        $validated = $request->validate([
            'deployment_status' => 'required|in:Successful,Failure'
        ]);

        $deployment->update(['deployment_status' => ucfirst($validated['deployment_status'])]);

        return Helper::sendResponse([], 'Deployment status updated', 201);
    }
