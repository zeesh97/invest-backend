...
use App\Http\Resources\SCRFResource;
...
class SCRFService extends BaseIndexService
{
    protected $model;
    ...
    {
        $this->model = SCRF::class;
        $this->resourceClass = SCRFResource::class;
    ...
    // public static function updateSCRF(array $storeData, array $uatScenariosData, $workflowId, $defined, $formId, SCRF $data)
    // {
    //     try {
    //         DB::beginTransaction();
    ...
    //             DB::commit();
    //             return Helper::sendResponse(new SCRFResource($result), 'Successfully Added', 201);
    //         } elseif ($isDraft === "false" && $wasReturn === false) {
    //             $result = GlobalFormService::processApprovals($data, $defined, $workflowId, $formId);
    //             DB::commit();
    //             return Helper::sendResponse(new SCRFResource($data), 'Successfully updated the record', 201);
    //         } else {
    //             DB::commit();
